...
class Asian < ActiveRecord::Base
  ...
  belongs_to :user
  has_many :asian_seasons, dependent: :destroy
  belongs_to :asian_genre
  ...
  default_scope -> { order('asians.updated_at DESC') }
  scope :by_asian_genre, -> (asian_genre_id) { where(asian_genre_id: asian_genre_id) }
  scope :latest_update, -> { unscope(:order).order('`asians`.updated_at DESC') }
